...
public sealed class OpenAIClientService(
    HttpClient httpClient,
    IOptions<OpenAISettings> openAISettings,
    ILogger<OpenAIClientService> logger) : IGenerativeAIClient
{
    private static readonly TimeSpan RequestTimeout = TimeSpan.FromSeconds(30);
    public ProvedorIA Provedor => ProvedorIA.OpenAI;
    ...
    public async Task<string> GerarConteudoAsync(string prompt, string apiKey, ConfiguracaoAnaliseIA? configuracaoIA, CancellationToken cancellationToken)
    {
        ...
        var model = string.IsNullOrWhiteSpace(configuracaoIA?.Modelo) ? openAISettings.Value.Model : configuracaoIA.Modelo.Trim();
        ...
        {
            throw new InvalidOperationException($"OpenAI retornou status {(int)response.StatusCode} usando o modelo '{model}'. Detalhes: {Shorten(responseContent)}");
        }

        var content = ExtractContent(responseContent);
        if (string.IsNullOrWhiteSpace(content))
        {
            throw new InvalidOperationException($"OpenAI retornou resposta sem conteudo textual usando o modelo '{model}'.");
        }
    ...
    public async Task<bool> TestarConexaoAsync(string apiKey, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "Falha ao testar conexao com OpenAI.");
            return false;
        }
    }
