...
public sealed class CohereClientService(
    HttpClient httpClient,
    IOptions<CohereSettings> cohereSettings,
    ILogger<CohereClientService> logger) : IGenerativeAIClient
{
    private static readonly TimeSpan RequestTimeout = TimeSpan.FromSeconds(30);
    public ProvedorIA Provedor => ProvedorIA.Cohere;
    ...
    public async Task<string> GerarConteudoAsync(string prompt, string apiKey, ConfiguracaoAnaliseIA? configuracaoIA, CancellationToken cancellationToken)
    {
        ...
        var model = string.IsNullOrWhiteSpace(configuracaoIA?.Modelo) ? cohereSettings.Value.Model : configuracaoIA.Modelo.Trim();
        ...
        {
            throw new InvalidOperationException($"Cohere retornou status {(int)response.StatusCode} usando o modelo '{model}'. Detalhes: {Shorten(responseContent)}");
        }

        var content = ExtractContent(responseContent);
        if (string.IsNullOrWhiteSpace(content))
        {
            throw new InvalidOperationException($"Cohere retornou resposta sem conteudo textual usando o modelo '{model}'.");
        }
    ...
    public async Task<bool> TestarConexaoAsync(string apiKey, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "Falha ao testar conexao com Cohere.");
            return false;
        }
    }
    ...
    private static string? ExtractContent(string responseContent)
    {
        try
        {
            ...
            if (root.TryGetProperty("text", out var legacyText) && legacyText.ValueKind == JsonValueKind.String)
            {
